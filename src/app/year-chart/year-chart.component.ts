...
export class YearChartComponent implements OnInit {
  ...
  onRefresh(chart:UIChart)
  {
    ...
    this.restConn.getConsumptionValuesHandler("feedin", "month", selDate).subscribe((response)=>{

      this.addDataset(response, 'Einspeisung [kWh]', 'rgba(129,201,91,0.85)', true);

      this.restConn.getConsumptionValuesHandler("production", "month", selDate).subscribe((response2)=>{

        this.addDataset(response2, 'Verbrauch aus Produktion [kWh]', 'rgba(14,169,252,0.85)', false);

        this.restConn.getConsumptionValuesHandler("consumption", "month", selDate).subscribe((response)=>{
  ...
  addDataset(response, type, color, minus)
  {
    ...
    for ( let entries of response )
    {
      ...
      let timeStr : String = String((dateLocal.getMonth()+1)).padStart(2, "0");
